    ...
    <div className="bg-white rounded-xl shadow-lg p-6 border">
        ...
        {documents.map((doc) => {
          ...
          return (
            <div key={doc.id} className="relative">
              ...
              {hasFile ? (
                <div className={`border-2 ${doc.borderColor} rounded-lg p-3 ${doc.bgColor}`}>
                ...
                <div className={`border-2 border-dashed ${doc.borderColor} rounded-lg p-4 text-center ${doc.bgColor} ${
                  errors[doc.id as keyof DocumentUploadData] ? 'border-red-500 bg-red-50' : ''
                } ${isDisabled ? 'opacity-50' : 'hover:border-opacity-60'}`}>
                  <div className={`w-12 h-12 mx-auto mb-3 ${doc.bgColor} rounded-lg flex items-center justify-center`}>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-center">
          ...
          <div className="text-lg font-bold text-red-600">{requiredDocs.length - uploadedRequiredCount}</div>
        </div>
        
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-center">
          <Upload className="w-5 h-5 text-yellow-600 mx-auto mb-2" />
          <div className="text-sm font-semibold text-yellow-800">Progress</div>
          <div className="text-lg font-bold text-yellow-600">{uploadedRequiredCount}/{requiredDocs.length}</div>
        </div>
        
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
          <CheckCircle className="w-5 h-5 text-green-600 mx-auto mb-2" />
          <div className="text-sm font-semibold text-green-800">Total</div>
          <div className="text-lg font-bold text-green-600">{totalUploaded}</div>
        </div>
      </div>
