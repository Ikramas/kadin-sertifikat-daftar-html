...
export default function Dashboard() {
  ...
  // const { toast } = useToast(); // <--- Dihapus untuk stabilitas
  ...
  // Bagian Notifikasi Pop-Up Toast Real-time (DIHAPUS UNTUK STABILITAS SEMENTARA)
  // useEffect(() => {
  //   if (isDocumentVerified) {
  //     const hasSeenVerifiedWelcome = localStorage.getItem('hasSeenVerifiedWelcome');

  //     if (!hasSeenVerifiedWelcome) {
  //       toast({
  //         title: 'Selamat Datang! Akun Berhasil Diverifikasi 🎉',
  //         description: 'Akun dan dokumen perusahaan Anda sudah 100% diverifikasi. Anda kini dapat mengajukan Sertifikat Badan Usaha Kadin.',
  //         variant: 'default',
  //         duration: 10000, 
  //         action: (
  //           <Link to="/applications">
  //             <Button variant="secondary" onClick={() => localStorage.setItem('hasSeenVerifiedWelcome', 'true')}>
  //               Ajukan SBU Sekarang
  //             </Button>
  //           </Link>
  //         ),
  //         onOpenChange: (open) => {
  //           if (!open) {
  //             localStorage.setItem('hasSeenVerifiedWelcome', 'true');
  //           }
  //         }
  //       });
  //     }
  //   } else {
  //     localStorage.removeItem('hasSeenVerifiedWelcome');
  //   }
  // }, [isDocumentVerified, toast]); 
