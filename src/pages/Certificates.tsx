...
const Certificates: React.FC = () => {
  ...
  // Fungsi untuk melihat/mengunduh sertifikat PDF
  // Menggunakan certificateId (yaitu certificate.id dari database) untuk identifikasi
  const handleViewCertificate = async (certificateId: string, certificateNumber: string) => {
    try {
      // Bentuk URL ke endpoint generate_pdf.php dengan certificate_id
      const fileUrl = `/backend/api/certificates/generate_pdf.php?certificate_id=${certificateId}`;
      
      // Menggunakan fungsi downloadAuthenticatedFile untuk membuka PDF di tab baru
      // Nama file yang diunduh akan menjadi "Sertifikat_SBU_[NomorSertifikat].pdf"
      // downloadAuthenticatedFile akan menangani penambahan token Authorization
      await downloadAuthenticatedFile(fileUrl, `Sertifikat_SBU_${certificateNumber}.pdf`);
      toast({
        title: 'Membuka Sertifikat',
        description: `Sertifikat ${certificateNumber} sedang dibuka di tab baru.`,
      });
    } catch (err: any) {
      console.error('Error viewing certificate:', err);
      toast({
        title: 'Gagal Membuka Sertifikat',
        description: err.message || 'Terjadi kesalahan saat mencoba membuka sertifikat.',
        variant: 'destructive',
      });
    }
  };
  ...
  const getStatusBadge = (status: string, category?: string) => {
    ...
    const statusConfig = {
      ...
      pending: { label: 'Dalam Proses', variant: 'outline' as const, icon: Clock, className: '' }, // For certificates status that are not active/expired/expiring
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-2">
          ...
          {certificate.status === 'active' ? (
            <Button 
              ...
              size="sm" 
              className="flex-1"
              // Mengirim certificate.id dan certificate.certificate_number
    ...
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            Gagal memuat data sertifikat: {error.message}. Silakan refresh halaman atau hubungi administrator.
          </AlertDescription>
        </Alert>
        <div className="mt-4">
