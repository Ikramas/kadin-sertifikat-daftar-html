...
import { downloadAuthenticatedFile } from '@/lib/utils'; // <-- Kita akan menggunakan ini
import { useErrorHandler } from '@/hooks/useErrorHandler'; 

interface CertificateData {
  id: string; // Ini adalah ID unik dari database
  certificate_number: string; // Nomor sertifikat (juga unik dan user-facing)
  ...
  company_name?: string; // Menambahkan ini agar bisa ditampilkan
}
...
const Certificates: React.FC = () => {
  ...
  const [loadingCertificateId, setLoadingCertificateId] = React.useState<string | null>(null); // State baru untuk loading

  const fetchCertificatesData = async (): Promise<CertificatesResponse> => {
  ...
  const { data: certificatesData, isLoading, error, refetch } = useQuery<CertificatesResponse, Error>({
  ...
  // --- PERBAIKAN FUNGSI handleViewCertificate untuk download langsung ---
  const handleViewCertificate = async (certificateId: string, certificateNumber: string) => {
    setLoadingCertificateId(certificateId); // Set loading state
    try {
      const fileUrl = `/backend/api/certificates/generate_pdf.php?certificate_id=${certificateId}`;
      const downloadFileName = `Sertifikat_SBU_${certificateNumber}.pdf`;
      ...
      handleSuccess(`Sertifikat ${certificateNumber} berhasil diunduh.`, 'Unduh Berhasil');
    } catch (err: any) {
      handleError(err, 'certificate_view_download', { certificateId, certificateNumber });
    } finally {
      setLoadingCertificateId(null); // Reset loading state
    }
  };
  // --- AKHIR PERBAIKAN ---
    ...
    <Card key={certificate.id} className="hover:shadow-md transition-shadow">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-2">
          {/* Tombol Lihat Sertifikat hanya aktif jika statusnya 'active' */}
          {certificate.status === 'active' ? (
            <Button 
              variant="default" 
              size="sm" 
              className="flex-1"
              onClick={() => handleViewCertificate(certificate.id, certificate.certificate_number)}
              disabled={loadingCertificateId === certificate.id} // Disabled saat loading
            >
              {loadingCertificateId === certificate.id ? (
                <RefreshCw className="w-4 h-4 mr-2 animate-spin" /> // Spinner saat loading
              ) : (
                <Download className="w-4 h-4 mr-2" /> // Ikon unduh
              )}
              {loadingCertificateId === certificate.id ? 'Memuat...' : 'Unduh Sertifikat'}
            </Button>
          ) : (
    ...
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            Gagal memuat data sertifikat: {error.message}. Silakan muat ulang halaman atau hubungi administrator.
          </AlertDescription>
        </Alert>
        <div className="mt-4">
    ...
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            Data sertifikat tidak tersedia. Mohon coba muat ulang halaman.
          </AlertDescription>
        </Alert>
        <div className="mt-4">
            <Button onClick={() => refetch()} variant="outline">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{certificatesData.summary.in_process_count}</p>
              </div>
