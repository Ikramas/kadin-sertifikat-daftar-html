...
export default function Settings() {
  ...
  const { handleError, handleSuccess, handleWarning } = useErrorHandler(); // Menambahkan handleWarning
  ...
  // Notification settings states (mocked persistence)
  const [emailNotifications, setEmailNotifications] = useState(
    JSON.parse(localStorage.getItem('emailNotifications') || 'true')
  );
  const [inAppNotifications, setInAppNotifications] = useState(
    JSON.parse(localStorage.getItem('inAppNotifications') || 'true')
  );

  // Theme settings state (mock)
  const [appTheme, setAppTheme] = useState(localStorage.getItem('appTheme') || 'system'); // 'light', 'dark', 'system'
  ...
  // Effect to persist notification settings locally
  useEffect(() => {
  ...
  // Effect to persist theme settings locally (for mock purposes)
  useEffect(() => {
    localStorage.setItem('appTheme', appTheme);
    // You might also need to apply a class to <html> or <body> for theme to work
    document.documentElement.className = appTheme === 'system' ? '' : appTheme;
  }, [appTheme]);
  ...
  const handleSaveUserProfile = async () => {
    ...
    try {
      if (!user) {
        throw new Error('Pengguna tidak terautentikasi. Mohon login ulang.');
      }
  ...
  const handleSaveCompanyProfile = async () => {
    ...
    try {
      if (!company) {
        throw new Error('Data perusahaan tidak tersedia. Mohon lengkapi dokumen registrasi.');
      }
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch('/backend/api/users/change-password.php', { 
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      if (data.status === 'success') {
        ...
        throw new Error(data.message || 'Gagal mengubah password. Pastikan password lama benar.');
      }
    } catch (error) {
      handleError(error, 'password_change');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteAccount = () => {
    ...
    handleWarning('Fitur hapus akun akan segera tersedia setelah implementasi backend yang aman dan konfirmasi lebih lanjut.', 'Hapus Akun');
  };
        ...
        <TabsContent value="profile" className="space-y-6">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setEditUserMode(!editUserMode)} disabled={isSaving}>
          ...
          {company && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => setEditCompanyMode(!editCompanyMode)} disabled={isSaving}>
        ...
        <TabsContent value="notifications" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-end pt-2">
                <Button onClick={() => handleSuccess('Pengaturan notifikasi disimpan.', 'Pengaturan Disimpan')} disabled={isSaving}>
                  <Save className="w-4 h-4 mr-2" />
        ...
        <TabsContent value="appearance" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-end pt-2">
                <Button onClick={() => handleSuccess('Pengaturan tampilan disimpan.', 'Pengaturan Disimpan')} disabled={isSaving}>
                  <Save className="w-4 h-4 mr-2" />
                  {isSaving ? 'Menyimpan...' : 'Simpan Pengaturan'}
                </Button>
              </div>
