...
const ApplicationDetail: React.FC = () => {
  ...
  const fetchApplicationDetail = async () => {
    if (!code_reg) {
      setError("Kode registrasi permohonan tidak ditemukan di URL.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Terjadi kesalahan saat memuat detail permohonan.');
      toast({
        title: 'Gagal Memuat Detail',
        description: err.message || 'Terjadi kesalahan saat memuat detail permohonan.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  }, [code_reg, isDocumentVerified, toast]);
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        fetchApplicationDetail(); // Refresh detail setelah submit untuk update status
      } else {
        toast({
      ...
      toast({
        ...
        description: err.message || 'Tidak dapat terhubung ke server untuk submit permohonan.',
        variant: 'destructive',
      });
    } finally {
  ...
  if (!application) {
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Alert variant="destructive">
          <XCircle className="h-4 w-4" />
          <AlertDescription>Detail permohonan tidak ditemukan.</AlertDescription>
        </Alert>
        <div className="mt-4">
          <Link to="/applications">
            <Button variant="outline">
