...
export default function DocumentRegistration() {
  ...
  const formatNPWP = (value: string) => {
    ...
    if (numbers.length > 9) formatted = `${formatted.substring(0,11)}-${numbers.substring(9)}`;
  ...
  const removeFile = async (index: number, documentId: string | null) => {
    ...
    if (documentId) {
        // --- PENTING: Endpoint ini belum ada di codebase backend yang Anda berikan.
        // Jika Anda ingin fungsionalitas hapus dokumen berfungsi penuh,
        // Anda perlu mengimplementasikan backend/api/documents/delete.php
        toast({
            title: 'Fitur Belum Tersedia',
            description: 'Penghapusan dokumen dari server saat ini tidak didukung. Mohon hubungi administrator jika perlu menghapus dokumen.',
            variant: "default", // Mengubah warning menjadi default untuk menghindari error tipe
        });
        console.warn("API for document deletion (backend/api/documents/delete.php) is not implemented."); //
        // --- Akhir catatan penting ---
        
        // Hapus secara lokal (tetapi di frontend tidak terlihat terhapus di backend)
        setDocuments(prev => prev.map((doc, i) =>
          i === index ? { ...doc, file: null, uploaded: false, backendId: null, fileUrl: undefined, original_name: undefined, error: null } : doc
        ));
        return; // Hentikan di sini jika API belum diimplementasikan
    }
