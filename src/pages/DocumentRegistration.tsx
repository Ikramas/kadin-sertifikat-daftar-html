...
export default function DocumentRegistration() {
  ...
  // State untuk menyimpan ID lokasi yang dipilih (untuk filtering dropdown berikutnya)
  ...
  const {
    ...
    setValue,
    watch,
    formState: { errors }, // Pastikan 'errors' dideklarasikan di sini
    reset,
  ...
  useEffect(() => {
    if (user && company) {
      // Mengisi form dengan data dari objek company.
      // Pastikan setiap field direset dengan nilai default jika null/undefined
      // Untuk lokasi, kita langsung pakai nama yang disimpan di DB
      reset({
        ...
        province: company.province || '', // Ini akan jadi nama provinsi (string) dari DB
        regencyCity: company.regency_city || '', // Ini akan jadi nama kab/kota (string) dari DB
        district: company.district || '', // Ini akan jadi nama kecamatan (string) dari DB
        village: company.village || '', // Ini akan jadi nama kelurahan (string) dari DB
      ...
      // Mengupdate state local untuk mengontrol dropdown bertingkat.
      // Kita perlu menemukan ID yang sesuai dengan NAMA yang dari DB, agar Select dapat menampilkan nama
      // dan filtering berikutnya berdasarkan ID.
      const foundProvince = mockLocations.provinces.find(p => p.name === company.province);
      setSelectedProvinceId(foundProvince ? foundProvince.id : '');

      const foundRegency = mockLocations.regencies[foundProvince?.id || '']?.find(r => r.name === company.regency_city);
      setSelectedRegencyId(foundRegency ? foundRegency.id : '');

      const foundDistrict = mockLocations.districts[foundRegency?.id || '']?.find(d => d.name === company.district);
      setSelectedDistrictId(foundDistrict ? foundDistrict.id : '');
      
      // Mengunci form jika status akun sudah pending verifikasi admin atau aktif
      if (['pending_admin_approval', 'active', 'verified'].includes(user.status)) {
        setIsSubmitted(true);
      }
    }
    
    // Mengisi status dokumen yang sudah diupload dari data user.documents
  ...
  const handleNumberInput = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Hanya izinkan angka, backspace, delete, tab, enter, arrow keys
  ...
  const removeFile = async (index: number, documentId: string | null) => {
    if (isSubmitted) return; // Tidak bisa menghapus jika form sudah disubmit
    ...
    if (documentId) {
        try {
            ...
            const response = await fetch(`/backend/api/documents/delete.php`, { 
                method: 'POST', // Gunakan POST jika DELETE tidak didukung oleh semua server
                headers: {
  ...
  const onSubmit = async (formData: DocumentRegistrationForm) => {
    ...
    try {
      ...
      const submissionData = {
        ...formData,
        // Data lokasi sudah dalam bentuk nama (string) karena SelectItem value sekarang adalah name
        // Jadi, tidak perlu lagi konversi find(p => p.id === formData.province)?.name di sini.
        // Backend harus menerima langsung nama provinsi/kab/kec/kel
      ...
      if (result.status === 'success') {
        setIsSubmitted(true);
        // Penting: Panggil fetchUserProfile untuk memperbarui AuthContext
        // Ini akan memastikan data user dan company di AuthContext adalah yang terbaru
        // Termasuk status verifikasi dan daftar dokumen yang sudah di-submit.
          ...
          <CardContent className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {/* Value prop untuk Select harus match dengan watch() */}
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                {/* Value yang disimpan adalah NAMA (string) */}
                <Select onValueChange={(value) => {
                  setValue('province', value, { shouldValidate: true });
                  // Temukan ID yang sesuai dengan NAMA untuk filtering dropdown berikutnya
                  const foundId = mockLocations.provinces.find(p => p.name === value)?.id || '';
                  setSelectedProvinceId(foundId); 
                  ...
                  <SelectContent>
                    {mockLocations.provinces.map((province) => (
                      <SelectItem key={province.id} value={province.name}> {/* value={province.name} */}
                        {province.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select onValueChange={(value) => {
                  ...
                  const foundId = mockLocations.regencies[selectedProvinceId]?.find(r => r.name === value)?.id || '';
                  setSelectedRegencyId(foundId); 
                  setValue('district', '', { shouldValidate: true }); 
                  setValue('village', '', { shouldValidate: true });
                  setSelectedDistrictId('');
                }} value={watch('regencyCity')} disabled={isSubmitted || !selectedProvinceId}>
                  <SelectTrigger className={errors.regencyCity ? 'border-destructive' : ''}>
                    <SelectValue placeholder="Pilih kabupaten/kota" />
                  </SelectTrigger>
                  <SelectContent>
                    {/* Menggunakan selectedProvinceId untuk filter */}
                    {mockLocations.regencies[selectedProvinceId]?.map((regency) => (
                      <SelectItem key={regency.id} value={regency.name}> {/* value={regency.name} */}
                        {regency.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select onValueChange={(value) => {
                  ...
                  const foundId = mockLocations.districts[selectedRegencyId]?.find(d => d.name === value)?.id || '';
                  setSelectedDistrictId(foundId); 
                  setValue('village', '', { shouldValidate: true });
                }} value={watch('district')} disabled={isSubmitted || !selectedRegencyId}>
                  <SelectTrigger className={errors.district ? 'border-destructive' : ''}>
                    <SelectValue placeholder="Pilih kecamatan" />
                  </SelectTrigger>
                  <SelectContent>
                    {/* Menggunakan selectedRegencyId untuk filter */}
                    {mockLocations.districts[selectedRegencyId]?.map((district) => (
                      <SelectItem key={district.id} value={district.name}> {/* value={district.name} */}
                        {district.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                {/* Value yang disimpan adalah NAMA (string) */}
                <Select onValueChange={(value) => setValue('village', value, { shouldValidate: true })} value={watch('village')} disabled={isSubmitted || !selectedDistrictId}>
                  <SelectTrigger className={errors.village ? 'border-destructive' : ''}>
                    <SelectValue placeholder="Pilih kelurahan" />
                  </SelectTrigger>
                  <SelectContent>
                    {/* Menggunakan selectedDistrictId untuk filter */}
                    {mockLocations.villages[selectedDistrictId]?.map((village) => (
                      <SelectItem key={village.id} value={village.name}> {/* value={village.name} */}
                        {village.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
