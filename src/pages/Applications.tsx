...
interface Application {
  ...
  code_reg: string; // <-- PASTIKAN INI TIDAK OPSIONAL KARENA KITA MENGANDALKANNYA
...
const NewApplicationForm: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const onSubmit = async (data: NewApplicationFormInputs) => {
    ...
    if (!neracaUploaded || !suratPermohonanUploaded) {
        toast({
            title: 'Dokumen SBU Belum Lengkap',
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        queryClient.invalidateQueries({ queryKey: ['applications'] }); // Invalidate dan refetch data aplikasi
        onClose();
...
const Applications: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery<ApplicationsResponse, Error>({
    ...
    refetchInterval: 1000 * 300, // Refetch setiap 5 menit (opsional, sesuaikan kebutuhan)
  });
      ...
      {showNewApplicationForm ? (
        <div className="mb-6">
          <NewApplicationForm onClose={() => {
            ...
            // queryClient.invalidateQueries(['applications']); // Sudah dihandle di NewApplicationForm sendiri
          }} />
        </div>
      ) : (
        <>
                ...
                <Card key={app.id} className="hover:shadow-md transition-shadow">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 pt-2">
                      ...
                      {/* Menggunakan app.code_reg untuk navigasi */}
