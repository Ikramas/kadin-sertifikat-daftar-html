...
import {
  Clock, // Untuk status pending
  CheckCircle, // Untuk status paid
  XCircle, // Untuk status failed/cancelled
  AlertTriangle, // Untuk peringatan (jika diperlukan)
  Receipt, // Ikon utama untuk daftar transaksi
  ArrowRight, // Ikon untuk tautan detail
  Info, // Untuk status refunded atau info umum
  Download // Import ikon Download
} from 'lucide-react';
...
const Transactions: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery<TransactionsResponse, Error>({
    ...
    enabled: isDocumentVerified, // Hanya fetch jika dokumen sudah diverifikasi
    staleTime: 1000 * 60, // Data dianggap stale setelah 1 menit
    refetchInterval: 1000 * 300, // Refetch setiap 5 menit
  });

  const transactions = data?.transactions || [];

  const handleDownloadInvoice = async (transactionId: string, transactionNumber: string) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error('Token autentikasi tidak ditemukan. Silakan login kembali.');
      ...
      // Perbaikan: Pastikan URL untuk generate_invoice_pdf.php benar
      // Jika Anda ingin mengunduh langsung dari backend, pastikan endpoint ini menangani pengunduhan
      // dan bukan menampilkan PDF di browser (Content-Disposition: attachment)
      const response = await fetch(`/backend/api/transactions/generate_invoice_pdf.php?transaction_id=${transactionId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'Gagal mengunduh faktur. Terjadi kesalahan jaringan atau server.';
        try {
      ...
      a.href = url;
      a.download = `Faktur_${transactionNumber}.pdf`;
      ...
      toast({
        ...
        description: `Faktur ${transactionNumber} berhasil diunduh.`,
      });
      ...
      toast({
        title: 'Unduh Gagal',
        description: err.message || 'Terjadi kesalahan saat mengunduh faktur. Pastikan server PDF generator sudah aktif.',
        variant: 'destructive',
      });
    }
  };
    ...
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Alert variant="destructive">
          ...
          <AlertDescription>Terjadi kesalahan: {error.message}</AlertDescription>
        </Alert>
        <div className="mt-4">
          ...
          {transactions.map((transaction) => (
            <Card key={transaction.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                {/* Tombol Unduh Faktur/Kwitansi - SELALU TAMPIL */}
                <div className="flex justify-end mt-4">
                  <Button
                    size="sm"
                    // Memanggil fungsi unduh faktur dengan ID dan nomor transaksi
                    onClick={() => handleDownloadInvoice(transaction.id, transaction.transaction_number)}
                  >
