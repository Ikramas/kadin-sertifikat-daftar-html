...
try {
    ...
    $userDocuments = [];
    // Pastikan kolom 'file_name' diambil dari database
    ...
    foreach ($docs_raw as $doc) {
        // INI BARIS KRUSIAL: Membentuk 'file_url' menggunakan 'file_name', BUKAN 'id'
        // Ini adalah nama file unik yang tersimpan di server
        // Pastikan $doc['file_name'] tidak kosong di database
        if (empty($doc['file_name'])) {
            error_log("Warning: Document ID " . $doc['id'] . " has empty file_name in DB. Cannot generate correct download URL.");
